# Subprocess-based conversion worker fallback for binary file processing.

"""Long-lived subprocess that imports MarkItDown once and converts files
on demand via IPC queues. This is the legacy fallback implementation.

Enable via SMART_SEARCH_SUBPROCESS_CONVERTER=1 environment variable.
The default in-process ConversionWorker is preferred; use this only as
a fallback if in-process conversion causes issues.

Usage:
    worker = SubprocessConversionWorker()
    worker.start()
...
class SubprocessConversionWorker:
    """Subprocess-based fallback for converting binary files to markdown.
    ...
    Use ConversionWorker (in-process) by default. Enable this via
    SMART_SEARCH_SUBPROCESS_CONVERTER=1 as a fallback only.
    """
